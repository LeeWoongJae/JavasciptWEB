document.addEventListener('DOMContentLoaded',function(){
    // ★DOM내용(페이지 전체내용)을 다 읽고 난후에 아래의 function을 실행★
    ...
    // 공란 기준으로 가각  나눈다 나눈것들은 인자로써 매겨진다
    console.log(strArry);
    let list = document.getElementById('list');
    strArry.forEach(function(val , ind){
    console.log(val);
        let li = document.createElement('li');
        let textN2 = document.createTextNode(val);
        li.appendChild(textN2);

        list.appendChild(li);

    })

    let listElems = document.getElementsByTagName('li');//list태그 전부를 가져온다
    console.log(listElems);
    let newStrAry = [];
    for(let item of listElems){
        console.log(item.innerText);//li의 텍스트값만 가져온다
        newStrAry.push(item.innerText);//각 li의 텍스트값들을 하나하나 새로운 배열의 마지막 인자로 계속 넣는다
    }
